...
const games = [
  {
    ...
    image:
      'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQSLkLnZ8qXhIt7ciwvIxPmtdXM_KDnQFPVQ&s',
  },
  {
    title: 'Skittles',
    ...
    image:
      'https://sporked.com/wp-content/uploads/2023/05/EXPLAINERS_SKITTLES_HEADER.jpg',
  },
  {
    title: 'Soccer Stars',
    ...
    image:
      'https://images.unsplash.com/photo-1599392227522-038b00c0db44?auto=format&fit=crop&w=800&q=80',
  },
  {
      ...
      <div className="container mx-auto px-4 py-12">
        <header className="text-center mb-12">
          ...
          <p className="text-gray-400">
            Your favorite casual games in one place
          </p>
        </header>
          ...
          {games.map((game, index) => (
            ...
            >
              <div className="aspect-square rounded-2xl overflow-hidden relative transition-transform transform hover:scale-105 hover:shadow-xl">
                {/* Top Left Corner Icon - Fully Visible & In Front */}
                <div className="absolute top-3 left-3 z-10 bg-white p-3 rounded-full shadow-2xl border border-gray-400">
                  {index === 0 || index === 1 || index === 3 ? (
                    <Star className="w-6 h-6 text-yellow-500" /> // Larger Star for first, second, and fourth game
                  ) : (
                    <ExternalLink className="w-6 h-6 text-blue-500" /> // External link for third game (Soccer Stars)
                  )}
                </div>
                ...
                />
                <div
                  className={`absolute inset-0 ${game.color} bg-opacity-80 transition-opacity group-hover:bg-opacity-90 flex flex-col items-center justify-center p-8`}
                >
